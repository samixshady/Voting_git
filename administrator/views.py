    ...
    def get_context_data(self, *args, **kwargs):
        ...
        for position in Position.objects.all():
            ...
            for candidate in Candidate.objects.filter(position=position):
                ...
                this_candidate_data['name'] = caesar_decrypt(candidate.fullname)
...
def dashboard(request):
    # Decrypt the logged-in admin's info
    request.user.first_name = caesar_decrypt(request.user.first_name)
    request.user.last_name = caesar_decrypt(request.user.last_name)
    request.user.email = caesar_decrypt(request.user.email)
    ...
    for position in positions:
        ...
        votes_count = []
        for candidate in Candidate.objects.filter(position=position):
            decrypted_name = caesar_decrypt(candidate.fullname)
            list_of_candidates.append(decrypted_name)
            votes = Votes.objects.filter(candidate=candidate).count()
...
def voters(request):
    raw_voters = Voter.objects.all()
    voters = []
    for voter in raw_voters:
        admin = voter.admin
        admin.first_name = caesar_decrypt(admin.first_name)
        admin.last_name = caesar_decrypt(admin.last_name)
        admin.email = caesar_decrypt(admin.email)
        voters.append(voter)
